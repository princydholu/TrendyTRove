          ...
          <tbody>
            {variant.sizes.map((s, sIdx) => (
              <tr key={sIdx}>
                ...
                <td style={{ paddingRight: "8px" }}><input type="number" placeholder="600" min="1" value={s.sellingPrice} onChange={(e) => updateSize(sIdx, "sellingPrice", e.target.value)} onKeyDown={(e) => ["-", "+", "e"].includes(e.key) && e.preventDefault()} style={{ ...S.input(errors[`variant_${vIdx}_price_${sIdx}`]), width: "100px" }} /><FieldError msg={errors[`variant_${vIdx}_price_${sIdx}`]} /></td>
